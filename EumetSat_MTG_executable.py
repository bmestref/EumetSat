...
parser.add_argument('--skip_night', action='store_true', help="Skip low sun angle scenes (default: True)")
parser.add_argument('--country', type=str, help="Country (spain, france, mallorca, etc...)")
parser.add_argument('--resize_factor', type=float, help="Resize factor (float, a float between 0 and 1)")
...
for product in products:
    ...
    for entry in product.entries:
        if any(pattern in entry for pattern in chunk_patterns):
            ...
            if skip_night:
                ...
                if sun_elev < 25: # 25º as threshold
    ...
    try:
        ...
        red_scaled_uint8_resized = cv2.resize(red_scaled_uint8, (int(img_width*resize_factor), int(img_height*resize_factor)), interpolation=cv2.INTER_AREA)
